...
#define IDLE 3
#define HIT 4
//redefinir macros do arquivo com base no sprite sheet escolhido

Zumbi::Zumbi(float x, float y) {
...
void Zumbi::atualizar() {
	if (isAlive()) {
		...
		GerenciadorDeColisao::colisao_zumbi(this);
...
void Zumbi::render() {
	...
	if (isAlive()) {
		...
		if (state == ATTACKING) {
			...
			frameNumber++;
			if (frameVal == 4 || frameVal == 8) { GerenciadorDeColisao::ataqueZumbi(this); }
